...
1. **Exporting Excalidraw diagrams to PNG/SVG** - High-quality image export with customizable settings
2. **Converting Mermaid diagrams to Excalidraw** - Instant conversion from Mermaid syntax to Excalidraw format

### Features

#### 📸 PNG Export
- Drag-and-drop or browse for `.excalidraw` files
- Live preview of the rendered diagram
- Export to PNG or SVG format
- Adjustable settings:
  - **Scale**: 1x-4x (2x recommended for presentations, 3x for print)
  - **Background Color**: Custom color picker (default: white)
  - **Padding**: 0-100px around diagram
  - **Format**: PNG or SVG

#### 🔄 Mermaid Converter
- Paste Mermaid diagram code
- Built-in examples (flowchart, sequence, class, gantt, pie)
- Instant conversion to Excalidraw JSON
- Export options:
...
```bash
# Windows
start export-to-png.html

# macOS
open export-to-png.html

# Linux
xdg-open export-to-png.html
```

**Option 2: From VS Code**
- Right-click the file → "Open with Live Preview"
- Or install "Live Server" extension and open with Live Server

### Workflow Examples

**Example 1: Export existing diagram to PNG**
1. Open `export-to-png.html` in browser
2. Ensure "PNG Export" tab is selected
3. Drag and drop your `.excalidraw` file
4. Adjust scale (2x recommended)
5. Click "Download PNG"

**Example 2: Create diagram from Mermaid and export**
1. Open `export-to-png.html` in browser
2. Switch to "Mermaid Converter" tab
3. Paste your Mermaid diagram code or load an example
4. Click "Convert & Export PNG"
5. Get both `.excalidraw` file AND PNG image!

**Example 3: Quick PNG for documentation**
```
1. Create diagram in Excalidraw
2. Save as .excalidraw file
3. Open export-to-png.html
4. Drop file, set scale to 2x
5. Download PNG
6. Insert PNG into docs/slides/wiki
```
...
- ✅ **Reliable rendering**: Uses official Excalidraw libraries
- ✅ **No dependencies**: Works in any modern browser (Chrome, Firefox, Safari, Edge)
- ✅ **Cross-platform**: Windows, macOS, Linux
- ✅ **Live preview**: See exactly what you'll get
- ✅ **Feature-rich**: Full control over export settings
- ✅ **Fast iterations**: Drag, adjust, export in seconds

### Recommended Export Settings

| Use Case | Scale | Format | Background | Padding |
|----------|-------|--------|------------|---------|
| PowerPoint/Google Slides | 2x | PNG | White or transparent | 20-30px |
| Printed documents | 3x-4x | PNG/SVG | White | 40px |
| Web documentation | 2x | PNG | White | 20px |
| Social media | 2x-3x | PNG | Custom | 30px |
| Technical diagrams | SVG | SVG | White | 20px |

### Integration with Agent Workflow

When generating diagrams for users, include complete export instructions:

```
Created: my-diagram.excalidraw

🎨 To Export as PNG:
1. Open: .agents/skills/excalidraw-diagram-generator/scripts/export-to-png.html
2. Drag and drop my-diagram.excalidraw
3. Set scale to 2x (or 3x for print quality)
4. Click "Download PNG"

Your PNG is ready for presentations, docs, or reports!
```

### Technical Details

The tool uses:
- **@excalidraw/excalidraw** library for PNG/SVG export
- **@excalidraw/mermaid-to-excalidraw** for Mermaid conversion
- Canvas API for rendering
- No server required - everything runs client-side

### Future: Command-Line Version

A Python-based CLI tool could be added for automation:
```bash
python excalidraw-to-png.py <input.excalidraw> [output.png] --scale 2
```

However, the current browser tool provides the most reliable and feature-complete experience.

---

## Summary

This directory contains four tools for working with Excalidraw diagrams:

1. **split-excalidraw-library.py** - Split icon libraries for efficient use
2. **add-icon-to-diagram.py** - Add professional icons to diagrams
3. **add-arrow.py** - Connect diagram elements with arrows
4. **export-to-png.html** - Export diagrams to PNG/SVG and convert from Mermaid

Together, these tools enable a complete workflow:
- Create diagrams manually or from Mermaid syntax
- Enhance with professional icons (AWS, GCP, etc.)
- Connect elements with labeled arrows
- Export to high-quality PNG/SVG for sharing
